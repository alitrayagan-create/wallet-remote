...
typedef void (*BleClipOutHandler)(const uint8_t* data, size_t len);
...
private:
  ...
  BLECharacteristic* inputClip = nullptr;
  BLECharacteristic* outputClip = nullptr;
  BLECharacteristic* featureClip = nullptr;
  ...
  BleClipOutHandler  clipOutHandler = nullptr;
  uint8_t            clipSeq = 0;

  uint16_t vid       = 0x303A;   // Espressif
  uint16_t pid       = 0x4011;   // Wallet Remote
...
public:
  ...
  void setClipOutHandler(BleClipOutHandler h);
  void sendClipRequest();
  void clearClipRequest();
protected:
